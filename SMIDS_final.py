...
class SMIDSSpermDataset(Dataset):
    """Expect folder layout: root/ClassA/*.bmp, root/ClassB/*.bmp, ..."""
    def __init__(self, root_dir, indices=None, transform=None, phase='train', target_balance=True):
        ...
        for cls in self.classes:
            ...
            imgs = [f for f in os.listdir(cdir) if f.lower().endswith('.bmp')]
            for name in imgs:
...
def predict_folder_with_best_pipeline(images_dir, save_dir, model_name, device=None, fold_dir=None):
    ...
    paths = [os.path.join(images_dir, f) for f in os.listdir(images_dir) if f.lower().endswith(".bmp")]
...
def run_cv(args):
    ...
    for c in classes:
        ...
        imgs = [f for f in os.listdir(cdir) if f.lower().endswith('.bmp')]
        for name in imgs:
            all_images.append(os.path.join(cdir, name)); all_labels.append(class_to_idx[c])
...
def parse_args():
    ...
    p.add_argument('--data_path', type=str, default='SMIDS-others')
    p.add_argument('--save_dir', type=str, default='result_SMIDS_160')
    p.add_argument('--epochs', type=int, default=160)  # 可改回100
