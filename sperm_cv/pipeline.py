# -*- coding: utf-8 -*-
"""Dataset-agnostic cross-validation pipeline shared between SMIDS and MHSMA."""
...
class MADRNet(nn.Module):
    def __init__(self, num_classes=3, backbone='resnet50', img_size=224,
...
class SpermDataset(Dataset):
    """Dataset wrapper configured via :class:`ExperimentConfig`."""
    ...
    def __init__(
        ...
        self.phase = phase
        self.rng = rng or np.random.default_rng(config.oversample_seed)
        ...
        all_images: List[str] = []
        all_labels: List[int] = []
        allowed_ext = tuple(ext.lower() for ext in config.image_extensions)
        for cls in self.classes:
            ...
            imgs = [
                ...
                and (not config.ignore_hidden or not f.startswith("."))
            ]
            for name in imgs:
    ...
    def _print_distribution(self) -> None:
        ...
        logger.info("📊 %s (%d):", self.phase.upper(), total)
        for i, name in enumerate(self.classes):
            cnt = class_counts.get(i, 0)
            pct = (cnt / total * 100) if total > 0 else 0.0
            logger.info("   %s: %d (%.1f%%)", name, cnt, pct)

    def __len__(self) -> int:
        return len(self.images)

    @staticmethod
    def enhance_sperm_features(image: np.ndarray) -> np.ndarray:
    ...
    def __getitem__(self, idx: int):
        ...
        if image is None:
            if img_path not in self._failed_reads:
                self._failed_reads.add(img_path)
...
def get_transforms(config: ExperimentConfig, phase: str = "train", img_size: int = 224):
    if phase == "train":
        return config.train_transform(img_size)
    return config.eval_transform(img_size)
...
def make_dataset(
    ...
    rng = np.random.default_rng(config.oversample_seed + seed_offset)
    transform = get_transforms(config, phase, img_size)
    return SpermDataset(
        config,
        root_dir,
        indices=indices,
...
def reproduce_best(
    ...
    if cfg is None:
        raise ValueError("Experiment configuration must be provided for reproduction")
    ...
    # val loader
    dataset_root = data_path or cfg.default_data_path
    ...
    val_loader = DataLoader(val_dataset, batch_size=cfg.default_batch_size, shuffle=False, num_workers=0, pin_memory=True)

    # CNN
    with open(os.path.join(base_dir, "checkpoints", f"{model_name}_model_config.json"), "r") as f:
        model_cfg = json.load(f)
    model = MADRNet(num_classes=model_cfg["num_classes"],
                                     backbone=model_cfg["backbone"], img_size=model_cfg["img_size"],
                                     use_rev_blocks=model_cfg["use_rev_blocks"],
                                     use_bilinear=model_cfg["use_bilinear"],
...
def predict_folder_with_best_pipeline(
    config: Optional[ExperimentConfig],
    images_dir: str,
    save_dir: str,
    model_name: str,
    ...
    cfg = config or CONFIG
    if cfg is None:
        raise ValueError("Experiment configuration must be provided for prediction")
    ...
    allowed_ext = tuple(ext.lower() for ext in cfg.image_extensions)
    paths = [
        os.path.join(images_dir, f)
        for f in os.listdir(images_dir)
        if f.lower().endswith(allowed_ext)
        and (not cfg.ignore_hidden or not f.startswith("."))
    ]
    class DummyDataset(Dataset):
        ...
        def __getitem__(self, i):
            ...
            if img is None:
                raise FileNotFoundError(f"Unable to read image at {paths[i]}")
            img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
            img = SpermDataset.enhance_sperm_features(img)
            pil = Image.fromarray(img)
            tensor = get_transforms(cfg, 'val', img_size)(pil)
            return tensor, paths[i]

    ds = DummyDataset()
    dl = DataLoader(ds, batch_size=cfg.default_batch_size, shuffle=False, num_workers=0, pin_memory=True)

    # load CNN
    with open(os.path.join(base_dir, "checkpoints", f"{model_name}_model_config.json"), "r") as f:
        cfg_model = json.load(f)
    model = MADRNet(num_classes=len(classes),
                                     backbone=cfg_model["backbone"], img_size=cfg_model["img_size"],
                                     use_rev_blocks=cfg_model["use_rev_blocks"],
                                     use_bilinear=cfg_model["use_bilinear"],
                                     pretrained=False).to(device)
    state = torch.load(os.path.join(base_dir, "checkpoints", f"{model_name}_best.pth"), map_location=device)
    model.load_state_dict(state); model.eval()
...
# Main with K-Fold CV
# -------------------------
def run_cv(config: ExperimentConfig, args):
    set_seed(config.oversample_seed)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    classes = sorted(
        [
            ...
            and (not config.ignore_hidden or not d.startswith("."))
        ]
    )
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(np.arange(len(all_images)), all_labels), start=1):
        ...
        for model_name, kwargs in model_specs:
            print(f"\n🧠 TESTING: {model_name}")
            model = MADRNet(num_classes=len(classes), backbone='resnet50', img_size=DEFAULT_IMG_SIZE,
...
# CLI
# -------------------------
def build_parser(config: ExperimentConfig) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description=f"Cross-validation pipeline for {config.name}")
    parser.add_argument('--data_path', type=str, default=config.default_data_path)
    parser.add_argument('--save_dir', type=str, default=config.default_save_dir)
    parser.add_argument('--epochs', type=int, default=config.default_epochs)
    parser.add_argument('--kfolds', type=int, default=config.default_kfolds)
    parser.add_argument('--batch_size', type=int, default=config.default_batch_size)
    parser.add_argument('--do_gradcam', type=int, default=1, help='1 to enable Grad-CAM visualisations')
    parser.add_argument('--do_tsne', type=int, default=1, help='1 to enable TSNE feature projections (first fold only)')
    return parser


def main(config: ExperimentConfig, argv: Optional[Sequence[str]] = None):
    """Entry-point used by dataset-specific scripts."""
    ...
    # reproduce_best(config, args.save_dir, "MADRNet_Full", fold_dir="fold_1", data_path=args.data_path)
    # preds = predict_folder_with_best_pipeline(config, "some_images_dir", args.save_dir, "MADRNet_Full", fold_dir="fold_1")
    # print(preds)
